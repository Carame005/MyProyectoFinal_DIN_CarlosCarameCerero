...
- RA1 (Interfaz y desarrollo):
  ...
  - RA1.b (crea interfaz gráfica): ver [`COMPONENTS.md`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/COMPONENTS.md) y los ficheros de pantallas en `app/src/main/java/.../pantallas/` (Login, Home, Tutor, Calendario). (Evidencia: definición de pantallas Compose y ejemplo de flujos UI.)
  - RA1.c (uso de layouts y posicionamiento): ver [`COMPONENTS.md`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/COMPONENTS.md) y `utils/` (componentes de layout). (Evidencia: uso de Column/Row/Box, ejemplos de jerarquía visual y cards de resumen.)
  - RA1.d (personalización de componentes): ver [`COMPONENTS.md`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/COMPONENTS.md) y `utils/*Componente.kt`. (Evidencia: componentes reutilizables y temas Material3 aplicados.)
  ...
  - RA1.g (asociación de eventos): ver `pantallas/CalendarioPantalla.kt`, `CalendarioAlarms.kt` y [`COMPONENTS.md`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/COMPONENTS.md). (Evidencia: creación/edición de eventos, scheduling y manejo de callbacks/acciones.)
...
- RA2 (NUI - Interfaces naturales):
  - RA2.a (herramientas NUI): ver [`NUI.md`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/NUI.md) y `security/BiometricUtils.kt`. (Evidencia: identificación de biometría, voz y detección como opciones y referencias a APIs.)
...
- RA3 (Componentes y reutilización):
  - RA3.a (herramientas de componentes): ver [`COMPONENTS.md`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/COMPONENTS.md) y `app/src/main/java/.../utils/`. (Evidencia: lista de componentes y librerías empleadas.)
  - RA3.b (componentes reutilizables): ver `COMPONENTS.md` y `utils/TutorComponente.kt`, `utils/TareaComponente.kt`. (Evidencia: cards y componentes que se reutilizan en varias pantallas.)
  - RA3.c (parámetros y defaults): ver `COMPONENTS.md` (descripción de API por componente). (Evidencia: parametrización y valores por defecto documentados.)
  - RA3.d (eventos en componentes): ver los callbacks documentados en `COMPONENTS.md` y su uso en `pantallas/`. (Evidencia: onClick, onStatusChange, onExpandChange y demás hooks.)
  - RA3.f (documentación de componentes): ver `COMPONENTS.md` y `documentacion/` en general. (Evidencia: documentación centralizada y ejemplos de uso.)
  - RA3.h (integración en la app): ver `pantallas/HomePantalla.kt`, `pantallas/TutorPantalla.kt`. (Evidencia: cómo los componentes se usan en múltiples pantallas.)
...
- RA4 (Usabilidad, estándares y evaluación):
  ...
  - RA4.c (menús): ver `utils/AppTopBar` / `SettingsDrawer` y `COMPONENTS.md`. (Evidencia: estructura de menús y accesos.)
  - RA4.d–e (distribución de acciones y controles): ver `USER_MANUAL.md` y `COMPONENTS.md`. (Evidencia: distribución en la UI, jerarquía de acciones y controles en pantallas principales.)
  - RA4.f (elección de controles): ver `DESIGN_JUSTIFICATION.md` y `COMPONENTS.md`. (Evidencia: justificación de tipos de control y su adecuación.)
  ...
  - RA4.i–j (pruebas de usabilidad y evaluación en dispositivos): ver `TESTS.md` y `DEV_SETUP.md` (instrucciones para pruebas). (Evidencia: plan de pruebas y notas sobre pruebas en dispositivos/emuladores.)
...
- RA5 (Informes):
  ...
  - RA5.b (generación de informes): ver `utils/ReportGenerator.kt` y `pantallas/Reports` (Home/Tutor). (Evidencia: funciones que construyen texto de informe y resumen numérico.)
  - RA5.c (filtros sobre valores): ver [`INFORMES.MD`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/INFORMES.MD) and the implementation of `ReportFilters` in `model/` or `utils`. (Evidencia: modelo de filtros y uso en UI.)
  - RA5.d (valores calculados/recuentos): ver `model/ReportSummary.kt` y `ReportGenerator`. (Evidencia: cálculos de totales y recuentos por estado.)
  - RA5.e (gráficos generados): ver `utils/ReportChart.kt` y la integración en `HomePantalla`/`TutorPantalla`. (Evidencia: componente gráfico y colores del tema aplicados.)
...
- RA7 (Distribución de aplicaciones - FFOE):
  ...
  - RA7.b (personalización del instalador): ver [`DEV_SETUP.md`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/DEV_SETUP.md) y `app/src/main/AndroidManifest.xml` (iconos, metadata, versionCode). (Evidencia: elementos a modificar para el instalador.)
  ...
  - RA7.e (firma digital): ver [`DEV_SETUP.md`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/DEV_SETUP.md) (firma y keystore) y `app/build.gradle.kts` (signingConfigs). (Evidencia: configuración y pasos para firmar releases.)
...
- RA8 (Pruebas avanzadas):
  ...
  - RA8.b (pruebas de integración): ver [`TESTS.md`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/TESTS.md) y los tests en `app/src/test/kotlin/...` (Robolectric/unit tests). (Evidencia: tests unitarios y de integración parcial; logs y reportes generados.)
  - RA8.c (pruebas de regresión): ver [`TESTS.md`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/TESTS.md) y `AppRepositoryTest.kt`. (Evidencia: tests de roundtrip y regresión sobre persistencia.)
  - RA8.d (pruebas de volumen/estrés): ver `VolumeTests.kt` (pruebas de carga moderada en persistencia). (Evidencia: tests que generan múltiples items y miden tiempo/tamaño.)
  - RA8.e (pruebas de seguridad): ver `CalendarioAlarmsSecurityTest.kt` y [`SECURITY.md`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/SECURITY.md). (Evidencia: tests que simulan excepciones y documentación de riesgos/mitigaciones.)
  - RA8.f (uso de recursos): ver `VolumeTests.kt` y observaciones en [`TESTS.md`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/TESTS.md). (Evidencia: mediciones básicas de tiempo/tamaño; sugerencias de ampliación.)
  - RA8.g (documentación de pruebas): ver [`TESTS.md`](https://github.com/Carame005/MyProyectoFinal_DIN_CarlosCarameCerero/blob/master/documentacion/TESTS.md) y los reportes en `app/build/reports/tests/`.
